...
"cargo loco generate model " + .title + " " +(.properties | to_entries | map({name: .key} + (.value | handle_nullable | map_custom_type)) | map(.name + ":" + .type + (if .nullable then "!" else "" end)) | join(" "))' article.json
